...
const counties = [{ code: 'UKR', name: 'Ukraine' }];
...
describe('Locations store tests', () => {
	beforeEach(() => {
		locationsInstance.countries = locationsInstance.serializeCountries(counties);
	...
	it('Check correct countries serialize', () => {
		const res = locationsInstance.serializeCountries(counties);
		const expectedData = {
	...
	it('Check correct init method call', () => {
		...
		expect(instance.init()).resolves.toEqual([counties, cities, airlines]);
	});
});
